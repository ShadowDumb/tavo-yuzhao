  function renderForum(state, nav, search, tag, player, ui) {
    ...
    if (view === 'post') {
      ...
      CORE.safeArray(forum.posts, 20).forEach(function (item) { if (String(item.id) === String(nav.params && nav.params.id)) post = item; });
       if (!post) return '<main class="yz-page-inner" data-marker="forum-post">' + yzHeader(t.features.forum, false) + '<div class="yz-empty">' + CORE.escapeHtml(t.guards.post) + '</div></main>';
      ...
      var postArmed = !!(ui && ui.armed && ui.armed.id === 'post:' + String(post.id));
        var postDeleteConfirm = playerDeleteConfirmFor('post', t);
        var editBtn = (player && isMine) ? '<div class="yz-form-actions"><button type="button" class="yz-send" data-action="entity-edit" data-kind="post" data-id="' + CORE.escapeHtml(post.id) + '" aria-label="' + CORE.escapeHtml(playerActionLabel(t.playerEdit, 'post', post.title, t)) + '">' + CORE.escapeHtml(t.playerEdit) + '</button>' +
          '<button type="button" class="yz-clear-btn' + (postArmed ? ' armed' : '') + '" data-action="entity-delete" data-kind="post" data-id="' + CORE.escapeHtml(post.id) + '" aria-label="' + CORE.escapeHtml(playerActionLabel(postArmed ? postDeleteConfirm : t.playerDelete, 'post', post.title, t)) + '"' + (postArmed ? ' data-wipe-base="' + CORE.escapeHtml(postDeleteConfirm) + '"' : '') + '>' + CORE.escapeHtml(postArmed ? postDeleteConfirm : t.playerDelete) + '</button></div>' : '';
      // 玩家域（公开数据）：底部评论输入框（data-comment-input 由 App 层绑定发送）。
       var commentBox = player
         ? '<div class="yz-composer"><div class="yz-input-wrap"><input type="text" data-comment-input data-post-id="' + CORE.escapeHtml(post.id) + '" placeholder="' + CORE.escapeHtml(t.playerCommentPlaceholder) + '" aria-label="' + CORE.escapeHtml(t.playerCommentPlaceholder) + '" maxlength="3000"><span class="yz-length-counter" data-length-counter="comment" aria-live="polite">0/3000</span></div>' +
      ...
      return '<main class="yz-page-inner yz-page-composer" data-marker="forum-post">' +
         yzHeader(t.features.forum, false) + searchBoxIf(allComments.length, search) +
         ...
         (CORE.hasText(post.section) ? '<span class="yz-tag">' + CORE.escapeHtml(forumSectionLabel(post.section)) + '</span>' : '') +
    ...
    var list = posts.length ? posts.map(function (post) {
      ...
      var row = '<b>' + CORE.escapeHtml(post.title) + (isMine ? ' <i class="yz-player-tag">' + CORE.escapeHtml(t.playerPostTag) + '</i>' : '') + '</b><em>' + CORE.escapeHtml((post.author || '') + (CORE.hasText(post.section) ? ' · ' + forumSectionLabel(post.section) : '')) + '</em><time>' + CORE.escapeHtml(post.time || '') + unreadBadge + '</time>';
       if (player && isMine) {
         return editableListRow(button('navigate', row, { view: 'post', id: post.id }, 'yz-manage-main yz-row-main'), 'post', post.id, false, post.title);
      }
    ...
    return '<main class="yz-page-inner" data-marker="forum-list">' + yzHeader(t.features.forum, false) + searchBoxIf(CORE.safeArray(state.forum && state.forum.posts, 20).length, search) + '<div class="yz-page-list">' + list + '</div>' + cta + '</main>';
  }
