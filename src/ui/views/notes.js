  function renderNotes(state, nav, search, player, ui) {
    ...
    if (view === 'note') {
      ...
      if (player) {
        ...
        actions = '<div class="yz-form-actions">' +
          '<button type="button" class="yz-send" data-action="entity-edit" data-kind="note" data-id="' + CORE.escapeHtml(rowNote.id) + '" aria-label="' + CORE.escapeHtml(playerActionLabel(t.playerEdit, 'note', rowNote.title, t)) + '">' + CORE.escapeHtml(t.playerEdit) + '</button>' +
          '<button type="button" class="yz-clear-btn' + (noteArmed ? ' armed' : '') + '" data-action="entity-delete" data-kind="note" data-id="' + CORE.escapeHtml(rowNote.id) + '" aria-label="' + CORE.escapeHtml(playerActionLabel(noteArmed ? t.playerDeleteConfirm : t.playerDelete, 'note', rowNote.title, t)) + '"' + (noteArmed ? ' data-wipe-base="' + CORE.escapeHtml(t.playerDeleteConfirm) + '"' : '') + '>' + CORE.escapeHtml(noteArmed ? t.playerDeleteConfirm : t.playerDelete) + '</button>' +
          '</div>';
      }
    ...
    if (view === 'folder') {
      ...
      var list = rows.length ? rows.map(function (note) {
        ...
        if (player) return editableListRow(button('navigate', inner, { view: 'note', id: note.id }, 'yz-manage-main yz-row-main'), 'note', note.id, false, note.title);
      ...
      var folderHasNotes = CORE.safeArray(notes.notes, 30).some(function (note) { return String(note.folderId) === String(folder.id); });
      return '<main class="yz-page-inner" data-marker="notes-list">' +
        yzHeader(CORE.escapeHtml(folder.name)) + searchBoxIf(folderHasNotes, search) + '<div class="yz-page-list">' + list + '</div>' + cta + '</main>';
    }
    var folderCards = CORE.safeArray(notes.folders, 10).filter(function (f) {
      ...
      return player ? editableListRow(main, 'folder', f.id, false, f.name) : button('navigate', '<span class="yz-folder-glyph">📁</span><span class="yz-row-copy"><b>' + CORE.escapeHtml(f.name) + '</b><em>' + CORE.escapeHtml(String(f.count || 0) + ' ' + t.labels.notesWord) + '</em></span>', { view: 'folder', id: f.id }, 'yz-row');
    });
