...
(function () {
  ...
  function callUi(name, event) {
    if (shared && shared.ui && typeof shared.ui.hooks === 'function') {
      var directHooks = shared.ui.hooks();
      var directFn = directHooks && directHooks[name];
      if (typeof directFn === 'function') {
        try { return Promise.resolve(directFn(event)); } catch (e) { return event; }
      }
      return event;
    }
    ...
    function notifyOpen() {
      try {
        if (typeof host.dispatchEvent === 'function') {
          host.dispatchEvent(new host.CustomEvent('yu-zhao:open'));
        }
      } catch (_) {}
      return callUi('open');
    }
    if (typeof plugin.onInputAction === 'function') plugin.onInputAction('open-jade', notifyOpen);
    if (typeof plugin.onSidebarAction === 'function') {
      plugin.onSidebarAction('open-jade', notifyOpen);
