...
> 基线：v3.0.0 之后的用户视角评审修复（R2/R3）+ UI 一致性重设计；冒烟 926 → 961 项全绿。

### 用户视角修复（R2 交互与流程 / R3 文案与反馈）
- **数据安全**：空间切换等待持久化结果、失败回滚并提示（R2-03）；未读游标保存失败反馈（R2-08）；单功能清空同步清除旧摘要/状态/记录（R2-04）；表单/导入/导航离开统一 dirty 守卫与放弃确认（R2-07、R2-15）
- **删除与确认**：删除后不再误落"新建表单"（R2-01）；删单条消息不再退出会话且刷新线程摘要（R2-02）；二次确认在重渲染下保留 armed 与倒计时（R2-05）；确认文案说明对象/级联/默认空间重建/6s 撤销窗口（R2-06、R3-04、R3-05）
- **列表与卡片**：消除嵌套卡片，改外层卡片 + 无背景主按钮（R2-10、R2-11）；联系人增加可见编辑入口（R2-14）；空间管理开关改用普通 toggle 样式、默认空间 AI 可写明确禁用（R2-09）
- **检索与空态**：论坛详情补评论搜索框（R3-01）；地图搜索纳入当前所在地（R2-12）；空玉册文件夹不再显示全局搜索框（R2-13）
- **焦点与无障碍**：Overlay 焦点陷阱覆盖全部可聚焦元素（R2-16）；确认框 ARIA 归位（R2-17）；整页重绘恢复焦点并宣布页面变化（R2-18）
- **移动端**：移除 320px 最小高度硬限制，按 visualViewport 适配键盘遮挡（R2-19）；多个小于推荐尺寸的触屏入口扩到 ≥44px（R2-20）
- **文案**：英文界面新建帖子按语言保存 section（R3-02）；订单列表不再泄漏内部状态值（R3-03）；错误 Toast 不截断并给下一步（R3-06）；长文本输入显示长度反馈（R3-07）；管理说明去术语、精简首屏（R3-08）

### UI 一致性重设计
- **设计令牌**：新增 `--yz-r-control/card/pill/badge` 与 `--yz-h-control`(44px)/`--yz-h-compact`(38px)，圆角与控件高度全面令牌化，杜绝就近字面量
- **形状收敛**：交互控件统一圆角矩形；胶囊仅用于真标签页与开关轨道；正圆仅留给头像/图标/FAB/卦位；"×"图标按钮族不再圆方混排
- **高度统一**：内容区单行控件一律 44px，废除 36/38/~34 意外值；同行控件强制等高
- **顶栏/页头**：返回 ‹ 与关闭 × 改幽灵图标按钮（去边框底色、20px 字形、44px 命中区）；空间切换改柔底胶囊；顶栏改 `align-items:center` 修正基线错位
- **框字比例**：控件字号上调（新建 15 / 发送·确认 14 / 标签·清空·开关·胶囊 13），按钮字距 2→1，独立筛选标签收高到 38px，消除"小字塞大框"
- **去冗余**：移除页头空间名标签 `.yz-header-tag`，空间名只在顶栏出现一次
- **新同步反馈**：移除卦位文本角标「有新同步」，改为对应卦位按钮边框呼吸式高亮动画，查看后自动解除
- **悬浮窗通知**：移除悬浮窗红点角标，悬浮玉佩水波纹特效改为仅在有新通知（未读消息或新同步分区）时显现
- **八卦字形**：子视图页头卦象图标字号上调至 32px，主盘与管理页同步放大，提升高分屏阴阳爻辨识度

### 文档
- 新增 `SPEC.md`：UI 落地与验收标准（令牌表、组件映射、可判定验收清单），与 `DESIGN.md`（设计方向）分工
- `README.md`：修正打包命令（版本号取自 manifest、先删旧包再压）；更新目录树与冒烟基线

## v3.0.0（2026-08-31）

### 改版：双域 → 用户空间

- **废除角色域/玩家域设计**：每份聊天改由「用户空间」承载——所有空间同构（玉牌/讯息/玉册/论坛/坊市/储物/舆图全套分区），只维护一套 UI
- **默认空间 = {{char}}**：AI 协议同步的自然落点；可删除，数据到达时自动重建；名字跟随角色
- **空间管理页**：新建（自定义名称）/改名/删除（两击确认 + 6s 撤销）/一键切换；顶栏空间按钮直达
- **按空间的 AI 开关**：「发送AI」控制该空间是否注入提示词基线；「AI可写」控制协议能否写入（拒写记 issue 回声）；默认空间强制 AI 可写
- **协议带空间参数**：turn 行第 6 字段 = 目标空间名（缺省=默认空间）；多空间基线以 `<yzc_* space="…">` 分组注入；非默认空间只接受 diff 轮（防全量轮抹除用户数据）
- **真实发言门禁扩展**：c- 前缀联系人、pm-N/pmg-N 消息、pmc-N 评论、owner=player 帖子在任何空间都不可被模型改写/删除/伪造
- **玩家联系人页改版**：「发送消息给角色」固定通道移除，改为「新增联系人」——自定义名称、任意空间直接收发；未读 = 用户发言后的尾随回复数（客户端 seen 游标）
- **旧数据迁移**：v1 状态读入即整体迁入默认空间（schemaVersion 2）；旧玩家域镜像/世界书数据自动升级为「我」空间并清除旧存储；跨域镜像通道（syncPlayerChannel/Groups/Posts、已读游标、投递重试）整体移除
- **持久化简化**：单存储键/单快照链承载全部空间（世界书 yz-psnap 分片退役）；导入仅接受当前 v3 空间结构并在确认后替换
- **P2 功能与体验修复**：修复空间路由、diff 可见性、未读累计、撤销容量与评论上限；补齐 meta-only 持久化、最终基线预算和双字段剥离；确认框、loading、窄屏键盘、滚动恢复、地图删除、stale 页面、诊断文案、跨 tab 开关同步、空间首页和触摸尺寸全部加固
- **P3 细节与视觉修复**：补齐开关/页签/封印状态的无障碍语义；新同步增加文字徽标并支持减少动态效果；移除管理页无效入口和首页重复空态；保留玉牌折叠状态；修正论坛评论总数和长标题断词
- **工程维护**：将源码按 Core、Protocol、i18n、Runtime、Prompt、Views、App 拆分到 `src/`；数据层、静态 shell、CSS、FAB、modal、UI 渲染与交互由 `ui/jade.html` 独立承载，`entry.js` 仅保留 Hooks/入口动作与共享桥
...
- **文案/术语优化**：M1 状态标签图标化、M2 已送达→已发出、M3 已归档→已清理、M6 公开→与角色共享、M7 传讯→输入消息、M19 数据块→法器同步数据、M20 注入提示词→参与剧情同步
- **空态合并**：M10 检索框条件渲染、M11 双空态合并为单个复合提示、M12 群聊空态加说明
- **删除确认统一**：M13 玉册详情页删除加两击保护、M15 论坛帖子加删除按钮（两击确认）
- **全类型删除功能**：联系人、群聊、单条消息、舆图行踪/地点均支持两击删除+6s撤销；角色域删除走 `deleteCharacterEntity`，本地生效
- **记事文件夹列表删除**：文件夹列表页直接显示删除按钮，无需进表单页
- **订单状态下拉化**：M14 自由文本→select 下拉（open/completed/cancelled）
- **设置页入口**：M17 加「打开玉兆管理」按钮
- **清除确认加宽**：M18 3s→5s 确认超时
- **toast/confirm 不重叠**：M21 showConfirm 时先 clearToast()
- **FAB 首次提示**：M22 长按复位一次性引导
- **返回按钮首页隐藏**：M23 stack 为空时隐藏
- **域切换显示当前**：M24 显示当前域名+下拉箭头
- **parseError 加操作按钮**：M25 toast 带「打开诊断」入口
- **overlay/confirm 过渡动画**：M26 opacity .2s ease 平滑过渡
- **generation:success catch 游标回退**：M29 catch 中恢复 playerReadCursor
- **import 语义校验**：M30 无玉兆特征字段的 JSON 拒收
- **CSS/布局优化**：L3 节点字号≥10px、L6 评论计数器、L7 textarea rows 4→6、L8 卦位 title 属性、L10 禁制标签精简、L11 数量−按钮0时禁用、L12 窄屏标签不溢出、L13 toast aria-live
